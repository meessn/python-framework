...
if __name__ == '__main__':
    ...
    #n_action = np.zeros((nQuad,nQuad))
    n_action[0,0] = -1  #scenario circular
    sent_action = n_action.flatten()

    collisions = 0
    aux2 = time.time()
    for i in range(100*n_episodes):   #each episode is 100 time steps
        print("step:",i)
        aux1 = time.time()
        obs = System.stepMultiAgent(sent_action)  #running a step
        n_action = np.ones((nQuad,nQuad)) - np.eye(nQuad)
        #n_action = np.zeros((nQuad, nQuad))
        #n_action[1, 1] = -1

        if i%100 == 99:
            n_action[0, 0] = -1  #for multiple episodes, 
