...
model.ineq = @(z, p) mpc_nonlinIneq_chance_slack(z, p, model.nObs);
% upper/lower bound for inequalities
model.hl = [-1, -1, 0.2,     zeros(1, model.nObs)]';
model.hu = [ 1,  1,   1, +inf*ones(1, model.nObs)]';

%% Objective function
model.objective  = @(z, p) mpc_objective_chance_slack(z, p, model.nObs);
model.objectiveN = @(z, p) mpc_objectiveN_chance_slack(z, p, model.nObs);
...
solver_name = strcat('FORCESNLPsolver_chance_slack_', num2str(model.nObs), '_', ...
...
%% Storing the solver
% create a folder
folder_name = ['./solver/chance_slack', '/Chance_Slack_Forces_', num2str(model.nObs), '_', ...
    num2str(model.N), '_', num2str(1000*model.dt)];
mkdir(folder_name);
