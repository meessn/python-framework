...
% where
%   s = [slack_env, slack_coll].                            (2) number of parameters
...
model.eq = @(z) RK2(z((model.nin + model.nslack + 1) : model.nvar), ...
    z(1 : model.nin), @crazyflie_dynamics, model.dt); %compare the bebop and crazyflie, look at different constraints +send email to Hai on ' how to modify the mpc' + tell we implemented the dynamics of the crazyflie. 
...
model.ineq = @(z, p) mpc_nonlinIneq_basic(z, p, model.nObs);
% upper/lower bound for inequalities
model.hl = [-1, -1, 0.2,     zeros(1, model.nObs)]';
model.hu = [ 1,  1,   1, +inf*ones(1, model.nObs)]';

%% Objective function
model.objective  = @(z, p) mpc_objective_basic(z, p, model.nObs);
model.objectiveN = @(z, p) mpc_objectiveN_basic(z, p, model.nObs);
